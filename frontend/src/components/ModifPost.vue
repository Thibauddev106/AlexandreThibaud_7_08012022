    ...
    <div class="modal-dialog">
      ...
      <div class="modal-content" v-if="editOption=='modify'">
        ...
        <div class="modal-body">
          <form enctype="multipart/form-data" action="/update" method="put">
            <div class="input-group mb-3">
              ...
              <br />
              <textarea class="input-text" id="inputNewText" type="text" :value="post.comment"></textarea>
            </div>
            <div class="input-group mb-3" v-if="post.image">
              <br />
              <img class="img-thumbnail" :src="post.image" />
              <button type="button" class="btn btn-danger mx-auto mt-1" @click='deleteImgAction'>Effacer image</button>
            </div>
        ...
        <div class="modal-footer">
          ...
          <button type="button" class="btn btn-primary" @click="updatePost">Sauvgarder</button>
        </div>
      </div>
      ...
      <div class="modal-content" v-else>
        ...
        <div class="modal-footer">
          <button type="button" class="btn btn-secondary" data-dismiss="modal">Annuler</button>
          <button type="button" class="btn btn-danger" @click="deletePost">Supprimer post</button>
        </div>
      </div>
    </div>
  </div>
...
export default {
  ...
  methods: {
    deletePost() {
      axios
        .delete(`http://localhost:5000/api/post/article/delete/${this.post.id}`, {
          headers: {
    ...
    updatePost() {
      ...
      if (newContent && this.deleteImg) {
        axios
          .put(
            `http://localhost:5000/api/post/article/update/${this.post.id}`,
            {
              comment: newInput,
              image: ""
            },
